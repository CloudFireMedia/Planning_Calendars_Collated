function collatePlanningMaterial() {
  ...
  var teamsSheetId = Config.get('PLANNING_CALENDAR_FOR_TEAMS_ID');
  var sourceSpreadsheet = SpreadsheetApp.openById(teamsSheetId);
  ...
  for (var m = 1; m <= 12; m++) {
    ...
    var totalDays = new Date(startYear, m, 0).getDate()
    monthOccurance = 0;
  ...
  for (var s in sourceSheet_allSheets) {
    ...
    if ((sourceSheet_currentSheet.getName() == "Instructions")) continue;
