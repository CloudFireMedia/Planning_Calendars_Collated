...
//  var promoDeadlinesSheet = SpreadsheetApp.openById(PROMOTION_DEADLINES_CALENDAR_IDxxx_).getSheetByName(PROMOTION_DEADLINES_CALENDAR_COMMUNICATIONS_DIRECTOR_MASTER_SHEET_NAME_); 
...
function exportData() {
  ...
  var collatedSheet = SpreadsheetApp
  .getActive()
  .getSheetByName(COLLATED_PROMOTION_PLANNING_CALENDAR_COLLATED_DATA_SHEET_NAME_);
  
  var pdcSheetId = Config.get('PROMOTION_DEADLINES_CALENDAR_ID');
  var promoDeadlinesSheet = SpreadsheetApp
  .openById(pdcSheetId)
  .getSheetByName(PROMOTION_DEADLINES_CALENDAR_COMMUNICATIONS_DIRECTOR_MASTER_SHEET_NAME_);
    
  var tierSpreadsheetId = Config.get('PROMOTION_FORM_RESPONSES_GSHEET_ID');  
  var tierDateSheet = SpreadsheetApp.openById(tierSpreadsheetId)
  .getSheetByName(TIER_DUEDATE_SHEET_NAME_);
...
//    promoDeadlinesSheet_eventComments.setValue(collatedSheet_eventComments.getValue());
  ...
  promoDeadlinesSheet.getRange("D:D").setNumberFormat('MM.DD');
  ...
  function getFirstEmptyRow() {
    ...
    var pdcSheetId = Config.get('PROMOTION_DEADLINES_CALENDAR_ID');
    var targetSpreadsheet = SpreadsheetApp.openById(pdcSheetId);
